...
def verifier_ctn():
    ...
    try:
        ...
        # 2️⃣ PAYS
                ...
                const inputFra = document.querySelector(selector);
                if (inputFra) {
                    inputFra.scrollIntoView({ block: "center", behavior: "smooth" });
                    inputFra.click(); // On clique directement sur l'input s'il est visible
                    return true;
                }
                return false;
            """
            ...
            if not step2:
                print(f"❌ Erreur : Impossible de trouver le bouton {PAYS_DEP}.")
